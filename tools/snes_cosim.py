...
MISMATCH_LOG = "cosim_mismatch.log"


def _mismatch_log(text):
    """Append the divergence report to cosim_mismatch.log (the exact checkpoint,
    subsystem split and field diff), so the first divergence survives the run."""
...
def report_divergence(a, b, cp):
    ...
    out(f"  {'sub':6} {'A='+a.name:24} {'B='+b.name:24}")
    split = []
    for s in SUBS:
        ...
        if va != vb and s in COMPARED:
            split.append(s)
        out(f"  {s:6} {va:24} {vb:24} {mark}")
    out(f"  compared subsystems that split: {split or '(none — check ruler/pc currency)'}")
