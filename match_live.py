#!/usr/bin/env python3
"""Correlate layer renders (from LIVE VRAM) against the SAME-INSTANT
screenshot to find the real (map, tile base) used by each BG."""
import json
import os
from PIL import Image

BASE = r"E:\Recompilador Super Nintendo\StarOceanTest2\build-trace"
vram = open(os.path.join(BASE, "verify_replay", "vram_live.bin"), "rb").read()
cgram = open(os.path.join(BASE, "verify_replay", "cgram_live.bin"), "rb").read()
scrolls = json.load(open(os.path.join(BASE, "ppu_lines_full.json")))
shot = Image.open(os.path.join(BASE, "live_frame.bmp")).convert("RGB")
sp = shot.load()
...
print("screenshot brightness sanity:")
for y in range(0, 224, 16):
    row = ""
    for x in range(0, 256, 4):
        s = sp[x, y]
        lum = (s[0] + s[1] + s[2]) // 3
        row += "#" if lum > 60 else ("." if lum < 15 else "+")
    print("%3d %s" % (y, row))

print()
for tm in (0x4800, 0x4C00, 0x5000, 0x5800):
