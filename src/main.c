...
/* Dev pad-marker (SNESRECOMP_PAD_MARKER=1): stamp every real controller
 * button press with the current frame. Edge-triggered here in the input
 * event path, so quick taps that per-frame polling would miss are always
 * caught. Used to ask the player to mark drop zones while a replay runs;
 * the presses are never injected into the guest. */
static void pad_marker_stamp(uint32 m) {
  static int s_pm = -1;
  if (s_pm < 0) {
    const char *_e = getenv("SNESRECOMP_PAD_MARKER");
    s_pm = (_e && _e[0] && _e[0] != '0') ? 1 : 0;
  }
  if (s_pm) {
    extern int snes_frame_counter;
    fprintf(stderr, "[padmark] frame=%d mask=%08x\n", snes_frame_counter, m);
  }
}
#endif
...
error_reading:;
    ...
    while (true) {
...
#ifndef SNESRECOMP_CLEAN_BUILD
        /* Pad-marker isolation: with SNESRECOMP_PAD_MARKER=1 the real pad
         * buttons are excluded from the guest while a replay runs (presses
         * are stamped by pad_marker_stamp() in the input event path).
         * Inactive without the env var. */
        uint32 pad_btns = g_pad_buttons;
        uint32 pad_marker_active = 0;
        {
            static int s_pm = -1;
            if (s_pm < 0) {
                const char *_e = getenv("SNESRECOMP_PAD_MARKER");
            ...
            pad_marker_active = (s_pm == 1);
        }
        uint32 inputs = g_input_state |
            (pad_marker_active ? 0 : pad_btns) |
        ...
        t_emu0 = SDL_GetPerformanceCounter();
#endif
        RtlRunFrame(inputs |
#ifndef SNESRECOMP_CLEAN_BUILD
                    (pad_marker_active ? 0 : GetActiveControllers()) |
#else
         ...
         * the frame. Dumps only when the env var is set; inert otherwise. */
        { extern void aot_prof_frame_end(int);
          extern int snes_frame_counter;
          aot_prof_frame_end(snes_frame_counter); }
#endif
#ifndef SNESRECOMP_CLEAN_BUILD
        t_draw1 = SDL_GetPerformanceCounter();
        {
...
#endif
                static int s_phase_int = -1;
                if (s_phase_int < 0) {
                    const char *_e = getenv("SNESRECOMP_PHASE_INTERVAL");
                    s_phase_int = _e && _e[0] ? atoi(_e) : 120;
                    if (s_phase_int < 1) s_phase_int = 1;
                }
                if (++s_phase_n >= s_phase_int) {
#ifdef SNESRECOMP_INTERP_PROFILE
                    ...
                    { static int s_pd_init = 0;
                      static double s_p_bq = 0, s_p_s1 = 0, s_p_dm = 0, s_p_ac = 0, s_p_pp = 0, s_p_aq = 0;
                      static double s_p_uw = 0, s_p_us = 0, s_p_ub = 0;
                      static uint64_t s_p_aq_c = 0, s_p_aq_cy = 0;
                      extern uint64_t bridgeq_prof_calls, apucyc_prof_calls;
                      extern double bridgeq_prof_ms, sdd1_prof_ms, dma_prof_ms,
                                     apucyc_prof_ms, ppu_prof_ms;
                      extern uint64_t aotq_prof_calls, aotq_prof_cycles;
                      extern double aotq_prof_ms;
                      extern uint64_t apuw_prof_calls, apus_prof_calls, apub_prof_calls;
                      extern double apuw_prof_ms, apus_prof_ms, apub_prof_ms;
                      double bq = bridgeq_prof_ms, s1 = sdd1_prof_ms,
                             dm = dma_prof_ms, ac = apucyc_prof_ms, pp = ppu_prof_ms,
                             aq = aotq_prof_ms, uw = apuw_prof_ms, us = apus_prof_ms,
                             ub = apub_prof_ms;
                      uint64_t aq_c = aotq_prof_calls, aq_cy = aotq_prof_cycles;
                      double emu_avg = s_phase_emu / s_phase_n;
                      fprintf(stderr, "  bridgeq=%.2fms(%.0f%%emu) sdd1=%.2fms dma=%.2fms apucyc=%.2fms ppu=%.2fms\n",
                              s_pd_init ? bq - s_p_bq : 0.0,
                              emu_avg > 0 && s_pd_init ? 100.0 * (bq - s_p_bq) / emu_avg : 0.0,
                              s_pd_init ? s1 - s_p_s1 : 0.0,
                              s_pd_init ? dm - s_p_dm : 0.0,
                              s_pd_init ? ac - s_p_ac : 0.0,
                              s_pd_init ? pp - s_p_pp : 0.0);
                      fprintf(stderr, "  aotq=%.2fms %llu calls, %.0f cyc/body | apuW=%.2fms apuS=%.2fms apuB=%.2fms\n",
                              s_pd_init ? aq - s_p_aq : 0.0,
                              (unsigned long long)(s_pd_init ? aq_c - s_p_aq_c : 0),
                              s_pd_init && (aq_c - s_p_aq_c) ? (double)(aq_cy - s_p_aq_cy) / (double)(aq_c - s_p_aq_c) : 0.0,
                              s_pd_init ? uw - s_p_uw : 0.0,
                              s_pd_init ? us - s_p_us : 0.0,
                              s_pd_init ? ub - s_p_ub : 0.0);
                      (void)bridgeq_prof_calls; (void)apucyc_prof_calls;
                      (void)apuw_prof_calls; (void)apus_prof_calls; (void)apub_prof_calls;
                      s_p_bq = bq; s_p_s1 = s1; s_p_dm = dm; s_p_ac = ac; s_p_pp = pp; s_p_aq = aq;
                      s_p_aq_c = aq_c; s_p_aq_cy = aq_cy;
                      s_p_uw = uw; s_p_us = us; s_p_ub = ub;
                      s_pd_init = 1; }
...
#endif /* !SNESRECOMP_CLEAN_BUILD (phase) */

        /* Slow-frame logger (dev, SNESRECOMP_SLOWFRAME_MS=<ms>): logs any
         * frame whose emulation + draw cost exceeds the threshold, with the
         * split, so individual hitches are caught instead of being averaged
         * into the phase report. */
#ifndef SNESRECOMP_CLEAN_BUILD
        {
            static int s_slow_on = -1;
            static double s_slow_ms = 0;
            if (s_slow_on < 0) {
                const char *_e = getenv("SNESRECOMP_SLOWFRAME_MS");
                s_slow_ms = _e && _e[0] ? strtod(_e, NULL) : 0.0;
                s_slow_on = 1;
            }
            if (s_slow_ms > 0.0) {
                double emu = (double)(t_emu1 - t_emu0) * 1000.0 /
                             SDL_GetPerformanceFrequency();
                double draw = (double)(t_draw1 - t_emu1) * 1000.0 /
                              SDL_GetPerformanceFrequency();
                extern int snes_frame_counter;
                if (emu + draw > s_slow_ms)
                    fprintf(stderr, "[slow] frame %d emu=%.2fms draw=%.2fms total=%.2fms\n",
                            snes_frame_counter, emu, draw, emu + draw);
            }
        }
#endif /* !SNESRECOMP_CLEAN_BUILD (slow) */

        /* Clean-exit control (dev, SNESRECOMP_EXIT_AT_FRAME=<n>): exits the
         * main loop normally at frame n so atexit dev dumps (interp_hist_dump,
         * PPU phase split) fire on demand. Inert without the env var. */
#ifndef SNESRECOMP_CLEAN_BUILD
        {
            static int s_exit_on = -1;
            static long s_exit_at = -1;
            if (s_exit_on < 0) {
                s_exit_on = 1;
                const char *_e = getenv("SNESRECOMP_EXIT_AT_FRAME");
                if (_e && *_e) s_exit_at = strtol(_e, NULL, 0);
            }
            if (s_exit_at > 0) {
                extern int snes_frame_counter;
                if (snes_frame_counter >= s_exit_at) goto quit;
            }
        }
#endif /* !SNESRECOMP_CLEAN_BUILD (exit-at-frame) */
