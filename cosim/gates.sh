#!/usr/bin/env bash
# Gate runner for the Star Ocean co-simulation A-side (SNES_COSIM.md
# "Validation gates"). Run AFTER building build-cosim/so_cosim.exe. ALL of
# these must pass before any A-vs-B (recomp-vs-interp816) result is
# trustworthy -- the whole point is that the tool cannot be silently wrong.
#
#   ROM=/path/so.sfc ./gates.sh gate1     # A-vs-A determinism: MUST be 0 divergence
#   ./gates.sh gate3                      # injected fault: MUST halt ~cp20, name 'ram'
#   ./gates.sh gate4                      # hash-vs-byte audit: no AUDIT-FAIL lines
set -u
HERE="$(cd "$(dirname "$0")" && pwd)"
EXE="$HERE/../build-cosim/so_cosim.exe"
ROM="${ROM:-$HERE/../build/Release/Star Ocean (Japan).sfc}"
COORD="python $HERE/../tools/snes_cosim.py"
A="$EXE \"$ROM\""

[ -x "$EXE" ] || { echo "build $EXE first (cmake --build build-cosim)"; exit 1; }
