...
def shot():
    ...
    gdi32.DeleteObject(hbmp); gdi32.DeleteDC(hdcMem); user32.ReleaseDC(hwnd, hdcWindow)
    return bytes(buf), hashlib.md5(bytes(buf)).hexdigest(), w, h

p1, h1, w, h = shot()
time.sleep(3)
p2, h2, _, _ = shot()

diff = sum(1 for i in range(0, len(p1), 4) if p1[i] != p2[i])
print(f'Hash1: {h1}')
print(f'Hash2: {h2}')
print(f'Diferentes pixels: {diff} ({100*diff/(w*h):.2f}%)')
print('RESULTADO:', 'FRAMES AVANZAN' if diff > 5000 else 'CONGELADO')
